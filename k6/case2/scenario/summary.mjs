...
const VARIANTS = ['naive', 'atomic', 'pessimistic', 'optimistic'];
...
console.log('\n=== Throughput & Retry ===');
console.log(
  `  ${'variant'.padEnd(12)}  ${'reqs'.padStart(7)}  ${'rps'.padStart(7)}  ${'applied/s'.padStart(10)}  ${'fail%'.padStart(7)}  attempts(opt)`,
);
for (const v of VARIANTS) {
  ...
  const attemptsAvg =
    v === 'optimistic' ? num('optimistic_attempts', 'avg') : null;
  const attemptsStr =
    v === 'optimistic'
      ? attemptsAvg == null
        ? '   --'
        : attemptsAvg.toFixed(2)
      : '   —';
  console.log(
    `  ${v.padEnd(12)}  ${fmtInt(reqs)}  ${rps == null ? '     --' : rps.toFixed(1).padStart(7)}  ${applRps == null ? '        --' : applRps.toFixed(1).padStart(10)}  ${fmtPct(failRate == null ? null : failRate * 100)}  ${attemptsStr}`,
  );
}
